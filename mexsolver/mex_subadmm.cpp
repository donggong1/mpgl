...
/*
This implementation is mainly for the graph guided generalized fused LASSO problem.
*/
/*
struct graph
{
...
int solve_x_cg(double *x, const double *ATA, const double *ATy, const double *zuI, const double *zuG, 
...
int solve_subadmm(double *x, double *z, double *u, double *dx, double *zu,
	const double *ATA, const double *ATy, int *supp_idx_list, int non_zero_num,
	...
	int cg_max_ite, double cg_tol2, double rho_rate)
{//*dx, *u, *z, *zu,
...
/*    memset(tmpx, 0, sizeof(double)*node_num);
	...
	for (admm_ite = 0; admm_ite < admm_max_ite; ++admm_ite)
	{
		...
		// x updating
		solve_x_cg(x, ATA, ATy, zu, zu+node_num, edge_head, edge_tail, edge_num, node_num, sI, rho, cg_max_ite, cg_tol2);
		/////////// specific operation for fused lasso
		memset(tmpx, 0, sizeof(double)*node_num);
		for (int i = 0; i < non_zero_num; ++i)
		{
...
{
	double *x_in, *x_out, *ATA, *ATy, *zuI, *zuG, *edge_head_tmp, *edge_tail_tmp;
	int *edge_head, *edge_tail, *supp_idx_list;
	int edge_num, node_num, non_zero_num,admm_max_ite, cg_max_ite;
	double sI, lambda, rho,admm_tol2, cg_tol2, cg_ite_out, rho_rate;

	x_in = (double*)mxGetPr(prhs[0]);
	ATA  = (double*)mxGetPr(prhs[1]);
	ATy  = (double*)mxGetPr(prhs[2]);
	supp_idx_list = (int *)mxGetPr(prhs[3]);
	non_zero_num = (int)(*(double *)mxGetPr(prhs[4]));
	edge_head = (int*)mxGetPr(prhs[5]); 
	edge_tail = (int*)mxGetPr(prhs[6]);
	edge_num = (int)(*(double*)mxGetPr(prhs[7]));
	node_num = (int)(*(double*)mxGetPr(prhs[8]));
	sI  = *(double *)mxGetPr(prhs[9]);
	lambda = *(double *)mxGetPr(prhs[10]);
	rho = *(double *)mxGetPr(prhs[11]);
	admm_max_ite = (int)(*(double *)mxGetPr(prhs[12]));
	admm_tol2 = *(double *)mxGetPr(prhs[13]);
	cg_max_ite = (int)(*(double *)mxGetPr(prhs[14]));
	cg_tol2 = *(double *)mxGetPr(prhs[15]);
	rho_rate = *(double *)mxGetPr(prhs[16]);
...
*/
	...
	solve_subadmm(x_out, z, u, dx, zu,
		ATA, ATy, supp_idx_list, non_zero_num, edge_head, edge_tail, edge_num, node_num, sI, 
		lambda, rho, admm_max_ite, admm_tol2, cg_max_ite, cg_tol2, rho_rate);
